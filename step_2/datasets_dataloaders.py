...
class CustomImageDataset(Dataset):
    ...
    def __init__(self, annotations_file, img_dir, transform=None, target_transform=None):
        self.img_labels = pd.read_csv(annotations_file) # pandas file retaining name and label
    ...
    def __getitem__(self, idx):
        img_path = os.path.join(self.img_dir, self.img_labels.iloc[idx, 0]) # the join just concatenates strings
...
def main():
    ...
    print(f"features type is {type(train_features)}") # a tensor containing all the batch images (64, 1, 28, 28) == (B, C, H, W)
    print(f'Each feature is of type {type(train_features[0])}') # a tensor containing only one image (1, 28, 28)
    print(f"labels type is {type(train_labels)}") # idem (label-array tensor)
    print(f"each label is of type {type(train_labels[0])}") # idem (a single-element tensor)
    print(f"Feature batch shape {train_features.size()}")
    print(f"Labels batch shape {train_labels.size()}")

    img = train_features[0].squeeze() # what the fuck is a squeeze ? is it for the tensor shape (redundant dimensions)?
