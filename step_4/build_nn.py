...
class MyNeuralNetwork(nn.Module):
    ...
    def __init__(self):
        ...
        self.flatten = nn.Flatten() # defines its own layer called "flatten" from the torch.nn method nn.Flatten()
...
def main():
    ...
    logits = model(X)
    pred_probab = nn.Softmax(dim=1)(logits) # notice how we can use (certain?) Layers (nn.Module) directly.
    ...
    print(hidden1.size())
    hidden1 = nn.ReLU()(hidden1)    # Why are some layers init in a variable, and others don't?
